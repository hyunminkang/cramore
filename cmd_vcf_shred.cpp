...
int32_t cmdVcfShared(int32_t argc, char** argv) {
  ...
  int32_t tmpSampleUnit  = 1000;
  int32_t tmpVariantUnit = 100000;
  
  std::string reg;
  bool writeAllInfo = false;
  std::vector<std::string> keepFormats;
  ...
  BEGIN_LONG_PARAMS(longParameters)
    ...
    LONG_STRING_PARAM("out-prefix", &out, "Prefix of output VCF file name")
    LONG_STRING_PARAM("out-suffix", &out, "Suffix of output VCF file name")
    LONG_STRING_PARAM("out-pattern", &out, "String oattern of output file where $SAMPLE_ID$ is replaced with the sample ID for each output VCF file. Cannot be used together with --out-prefix or --out-suffix") 

    LONG_PARAM_GROUP("Options for temporary files", NULL)
    LONG_STRING_PARAM("tmp-prefix",&tmpPrefix,"Prefix of temporary files. If empty, it will be set to the output file name where $SAMPLE_ID$ is replaced with __TMP__");
    LONG_INT_PARAM("tmp-sample-unit",&tmpSampleUnit,"Unit of tempoary sample size to load into memory");
    LONG_INT_PARAM("tmp-variant-unit",&tmpVariantUnit,"Unit of temporary variant count to load into memory");
    
    LONG_PARAM_GROUP("Other options", NULL)
    LONG_STRING_PARAM("region",&reg,"Genomic region to focus on")
    LONG_PARAM("write-all-info",&writeAllInfo, "Write all INFO field. By default, only AC and AN will be recorded in each VCF");
    LONG_MULTI_STRING("keep-format", &keepFormats, "FORMAT field to keep from the original VCF. If empty, it will keep all format fields. if specified, it will keep only specified fields")
  END_LONG_PARAMS();
  ...
  if ( ( inVcfs.size() == 0 ) || ( inVcfList.empty() ) ) 
    error("[E:%s:%d %s] Missing required option (--vcf or --vcf-list)");

  if ( outPattern.empty() ) {
    if ( outPrefix.empty() && outSuffix.empty() ) 
      error("[E:%s:%d %s] Missing required option (--out-prefix,--out-suffix or --out-pattern)");
    outPattern = outPrefix + "$SAMPLE_ID$" + outSuffix;
  }

  std::vector<std::string> outFixes;
  uint32_t pos = 0, newpos;
  while( ( newpos = outPattern.find("$SAMPLE_ID$", pos) )
	 != std::string:npos ) {
    outFixes.push_back(outFixes.substr(pos,newpos-pos));
    pos = newpos + 11;
  }
  if ( newpos != std::string::npos )
    error("Unexpected value %u in newpos, newpos");
  outFixes.push_back(outFixes.substr(pos));

  if ( outFixes.size() <= 1 )
    error("Ouput pattern %s does not contain any '$SAMPLE_ID$', so cannot be distinguished");
  ...
  // read VCF and write temporary files  
  if ( tmpPrefix.empty() ) {
    tmpPrefix = outFixes[0];
    for(int i=1; i < (int)outFixes.size(); ++i) {
      tmpPrefix += "__TMP__";
      tmpPrefix += outFixes[i];
    }
  }

  // single-sample VCF will be stored, with temporary files storing the fields to store for a batch of samples
  BCFOrderedWriter* odw = new BCFOrderedWriter(out);
  
  for(int32_t i=0; i < (int32_t)inVcfs.size(); ++i) {
    notice("Loading input VCFs %d", inVcfs[i].c_str());
    BCFOrderedReader* odr = new BCFOrderedReader(inVcfs[i], intervals);
  }
