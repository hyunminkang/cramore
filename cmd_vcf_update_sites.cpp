...
int32_t cmdVcfUpdateSites(int32_t argc, char** argv) {
  ...
  genomeLocus* pLocus = NULL;
  if ( !dbsnpVcf.target_region.empty() ) {
    //siteVcf.target_region = dbsnpVcf.target_region;
    pLocus = new genomeLocus(dbsnpVcf.target_region.c_str());
  }
  ...
  while( ( sread = siteVcf.read() ) ) {
    ...
    //notice("%d %d %d",sv->pos,pLocus->beg1,pLocus->end0);    

    if ( pLocus ) {
      if ( sv->pos + 1 < pLocus->beg1 ) continue;
      if ( sv->pos + 1 > pLocus->end0 ) break;
    }
    ...
    */
    ...
    if ( ( !sread ) || (!gread) )
      error("[E:%s:%d %s] Two input VCF files do not share exact site list. sread = %d, gread = %d at (%d:%d %d)", __FILE__, __LINE__, __PRETTY_FUNCTION__, sread, gread, sv->rid, sv->pos, sv->rlen);
